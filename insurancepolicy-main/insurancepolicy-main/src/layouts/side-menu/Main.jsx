...
import {
  ...
  usersideMenu as userSideMenuStore,
...
function Main() {
  ...
  const [formattedUserMenu, setFormattedUserMenu] = useState([]);
  const sideMenuStore = useRecoilValue(useSideMenuStore);
  const UsersideMenuStore = useRecoilValue(userSideMenuStore);
  const sideMenu = () => nestedMenu($h.toRaw(sideMenuStore.menu), location);
  const usersideMenu = () =>
    nestedMenu($h.toRaw(UsersideMenuStore.menu), location);

  const userType = useSelector(
    (state) => state.user.value.user.authorities[0].authority
  );
  useEffect(() => {
  ...
  useEffect(() => {
    dom("body").removeClass("error-page").removeClass("login").addClass("main");
    setFormattedUserMenu(usersideMenu());
  }, [UsersideMenuStore, location.pathname]);
  return (
    <>
      {userType == "ADMIN" && (
        <div className="py-2">
        ...
        <div className="py-2">
          ...
          <div className="flex mt-[4.7rem] md:mt-0">
            ...
            <nav className="side-nav">
              ...
              <ul>
                {/* BEGIN: First Child */}
                {formattedUserMenu.map((menu, menuKey) =>
                  menu == "devider" ? (
                    <li
                      className="side-nav__devider my-6"
                      key={menu + menuKey}
